...
*/
void setup(network *inet, resgraph *onet){

    omp_lock_t active_lock;
    omp_init_lock(&active_lock);

    cout << "start of setup func" << endl;
    onet->src = inet->src;
    onet->sink=inet->sink;
    onet->n = inet->n;
    onet->m = inet->m;
...
void pulse(resgraph *net_ptr){
    cout << "ready to setup locks" << endl;
    omp_lock_t active_lock;
    omp_lock_t active_p_lock;
    omp_init_lock(&active_lock);
    omp_init_lock(&active_p_lock);

    cout << "ready to pulse" << endl;

    int v,w,r,ch,e,d_p,dw;
    #pragma omp parallel  private (v,w,r,ch,e,d_p,dw) shared (net_ptr)
    {
    # pragma omp for
    ...
    # pragma omp for
    for (int i=0; i<net_ptr->active.size(); i++){
        v = net_ptr->active[i];
        net_ptr->hght_p[v] = 4*(net_ptr->n); // shouldn't ever be bigger!!
        e = net_ptr->ex[v];
    ...
    for (int v=0; v<net_ptr->n; v++){
        ...
        if ( (net_ptr->ex[v] > 0) && (v!=net_ptr->src) && (v!=net_ptr->sink)){
            ...
            omp_set_lock(&active_p_lock);
            net_ptr->active_p.push_back(v);
            omp_unset_lock(&active_p_lock);
        }
    }
    #pragma omp for
    for (int i=0; i<net_ptr->active.size(); i++){
    ...
    net_ptr->active.clear();
    net_ptr->active_p.swap(net_ptr->active);
}
